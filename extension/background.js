...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'getScanData') {
    chrome.storage.local.get(['extensionRisks', 'currentSiteScan'], (result) => {
      const risks = result.extensionRisks || [];
      let siteScan = result.currentSiteScan || null;
      let isStaleData = false;
      
      // Check if cached site scan matches current URL
      if (request.currentUrl && siteScan && siteScan.url) {
        try {
          const cachedUrl = new URL(siteScan.url).hostname;
          const currentUrlHostname = new URL(request.currentUrl).hostname;
          
          // If URLs don't match, data is stale
          if (cachedUrl !== currentUrlHostname) {
            console.log('Tab changed - data is stale, triggering fresh scan');
            isStaleData = true;
            
            // Trigger content script to rescan current page
            if (request.tabId) {
              chrome.tabs.sendMessage(request.tabId, { action: 'rescanPage' }, (response) => {
                if (chrome.runtime.lastError) {
                  console.log('Could not trigger rescan:', chrome.runtime.lastError.message);
                }
              });
            }
            
            // Return current cached data but mark as stale
            // Don't replace with placeholder - let popup show "Scanning..." message
            siteScan = null;
          }
        } catch (e) {
      ...
      sendResponse({
        ...
        siteScan: siteScan, // Include website safety analysis
        totalExtensions,
        criticalCount: risks.filter(r => r.riskLevel === 'CRITICAL').length,
        highCount: risks.filter(r => r.riskLevel === 'HIGH').length,
        networkAlerts: networkAlerts.slice(-10), // Last 10 alerts
        suspiciousDownloads: suspiciousDownloads.slice(-10) // Last 10 downloads
      });
    });
    return true;
  }
    ...
    (details) => {
      ...
      if (details.initiator && details.initiator.startsWith('chrome-extension://')) {
        ...
        for (const domain of MALICIOUS_DOMAINS) {
          if (url.includes(domain)) {
            const alert = {
              ...
              description: `Extension contacted known malicious domain: ${domain}`
            };
            ...
            if (chrome.notifications) {
              chrome.notifications.create({
                ...
                title: '🚨 Security Alert: Malicious Domain',
                message: `Extension contacted ${domain}`,
                priority: 2
              });
            }
          }
        ...
        if (details.method === 'POST' && details.requestBody) {
          ...
          if (bodySize > 10000) { // >10KB POST request
            const alert = {
              ...
              url: details.url,
              dataSize: bodySize,
              description: 'Large POST request detected (possible data theft)'
            };
            
            networkAlerts.push(alert);
            console.warn('⚠️ NETWORK ALERT:', alert);
          }
        }
      }
...
if (chrome.downloads) {
  chrome.downloads.onCreated.addListener((downloadItem) => {
    ...
    const suspiciousExtensions = ['.exe', '.bat', '.cmd', '.ps1', '.vbs', '.js', '.jar', '.msi', '.scr'];
    const isSuspicious = suspiciousExtensions.some(ext => filename.endsWith(ext));
    
    // Check if download initiated by extension
    const isExtensionInitiated = url.startsWith('chrome-extension://');
    
    if (isSuspicious || isExtensionInitiated) {
      const alert = {
        ...
        dangerType: downloadItem.danger,
        description: isSuspicious 
          ? `Executable file download detected: ${filename}`
          : 'Extension initiated download'
      };
      
      suspiciousDownloads.push(alert);
      console.warn('⚠️ DOWNLOAD ALERT:', alert);
      
      // Notify user for executables
      if (isSuspicious && chrome.notifications) {
        chrome.notifications.create({
          type: 'basic',
          iconUrl: 'icons/icon128.png',
          title: '⚠️ Security Alert: Executable Download',
          message: `${filename} - Verify before running!`,
          priority: 2
        });
      }
    }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'getNetworkAlerts') {
    sendResponse({ networkAlerts, suspiciousDownloads });
    return true;
  }
