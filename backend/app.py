...
def report_risk():
    ...
    try:
        ...
        employee_id = sanitize_string(data.get('employee_id', ''))
        if not employee_id or len(employee_id) < 3 or len(employee_id) > 100:
            return jsonify({'error': 'Invalid employee_id format'}), 400
        
        # Validate extension_id format (alphanumeric only)
        ...
        if not isinstance(extensions, list) or len(extensions) > 100:
            return jsonify({'error': 'Invalid extensions data'}), 400
        ...
        for ext in extensions:
            ...
            if risk_level in ['HIGH', 'CRITICAL'] or len(flags) > 0:
                ...
                ext_id = sanitize_string(str(ext.get('id', '')))[:50]
                ext_name = sanitize_string(str(ext.get('name', 'Unknown')))[:200]
                ext_version = sanitize_string(str(ext.get('version', '')))[:50]
                
                # Validate risk_score is integer 0-100
                try:
                ...
                # Sanitize list data
                perms_sanitized = [sanitize_string(str(p), 100) for p in perms[:50]]
                hosts_sanitized = [sanitize_string(str(h), 200) for h in hosts[:20]]
                threats_sanitized = []
                for t in ext.get('threats', [])[:10]:
                ...
                for f in flags[:20]:
                    if isinstance(f, dict):
                        flags_sanitized.append({
                            'id': sanitize_string(str(f.get('id', '')), 20),
                            'severity': sanitize_string(str(f.get('severity', '')), 20),
                            'title': sanitize_string(str(f.get('title', '')), 100)
                        })
                
                # Validate install_type (whitelist)
                install_type = str(ext.get('install_type', 'unknown'))
                allowed_types = ['normal', 'development', 'sideload', 'other', 'unknown']
                ...
                try:
                    ...
                    ''', (
                        ...
                        ext_name,
                        risk_score,
                        json.dumps(perms_sanitized),
                        json.dumps(hosts_sanitized),
                        json.dumps(threats_sanitized),
                        json.dumps(flags_sanitized),
                        install_type,
                        enabled,
                        ext_version,
